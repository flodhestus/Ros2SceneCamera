# ROS2 Scene Camera

Unreal Engine 5.7 plugin: a **normal scene camera** that captures the view, runs a **compute shader** to pack RGB8, publishes **`sensor_msgs/Image`** over **CycloneDDS**, and shows a **live preview window** when you press **Play**.

Repository: [github.com/flodhestus/Ros2SceneCamera](https://github.com/flodhestus/Ros2SceneCamera)

Depends on **[Ros2DdsShared](https://github.com/flodhestus/Ros2DdsShared)** for DDS (no duplicate CycloneDDS copy in this plugin). **Not** a LiDAR plugin — no `Lidar360` naming.

Runs with **either** Gpu LiDAR or OptiX LiDAR on the **same DDS participant**.

## On Play (PIE)

| Role | Actor | DDS topic (default) | Window |
|------|--------|---------------------|--------|
| Publish | `ARos2SceneCameraPublisher` | `rt/sensor_image` | — |
| Subscribe | `ARos2SceneCameraSubscriber` | `rt/sensor_image` | **Scene Camera** |

Both actors spawn automatically when you press **Play**.

## GPU path

1. **`USceneCaptureComponent2D`** renders **Final Color LDR** to a **1920×1080** `PF_B8G8R8A8` render target (Full HD default).
2. **`SceneCameraCapture.usf`** compute shader (`16×16` threads, texel `Load`) writes **RGB8** via byte-address UAV — compute shader writes directly into the pooled DDS sample buffer when possible.
3. **`sensor_msgs/Image`** (`encoding: rgb8`) is published via shared `FLidar360Dds` (default QoS).
4. Subscriber decodes the image and draws it in an OpenGL Win32 window.

If the compute shader is not available, a CPU **ReadPixels** fallback is used.

## Performance

| Setting | Default | Notes |
|---------|---------|--------|
| Resolution | **1920×1080** | Full HD rgb8 ≈ **6.2 MB**/frame |
| Publish rate | **20 Hz** | Tick-driven; raise only if GPU headroom allows |
| GPU path | Pooled readback, no per-frame alloc | `bCaptureEveryFrame` keeps RT hot |
| DDS | Single participant + async publish | No duplicate init with LiDAR plugins |

## Quick start

1. Enable **ROS2 DDS Shared** and **ROS2 Scene Camera**.
2. Optionally enable **LiDAR 360 GPU Ray Tracing** or **LiDAR 360 OptiX**.
3. Press **Play**.

## Related plugins

- [Ros2DdsShared](https://github.com/flodhestus/Ros2DdsShared)  
- [Lidar360GpuRayTracing](https://github.com/flodhestus/Lidar360GpuRayTracing)  
- [Lidar360OptiX](https://github.com/flodhestus/Lidar360OptiX)  
